...
class Api:
    def __init__(self):
        self.base_url = 'https://dev.dsync.app:6443/api/Identity/getOTP'
        self.headers = {'Content-type': 'application/json', 'Accept': 'application/json'}

    def _assert_request(self, method: str, url: str, data=None, headers=None, status_code=202):
        response = requests.request(method, url, data=data, headers=headers)
        assert response.status_code == status_code, f'Status code is {response.status_code}'
        return response.json()
    def gen_otp(self, phonenumber:str):
        otp = generate_int
        data = [{
